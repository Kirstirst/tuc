...
class AgentsList extends CBitrixComponent implements Controllerable, Errorable
{
    ...
    final public function onPrepareComponentParams($arParams): array
    {
        ...
        if (!$this->arParams["AGENT_COUNT"]){
            $this->arParams["AGENT_COUNT"] = 20;
        }
    ...
    final public function executeComponent(): void
    {
        ...
        if ($this->cache->initCache(
            ...
            $arTypeAgents = self::getFieldListValue($arHlblock, 'UF_WORKING'); // получить массив со значениями списочного свойства Виды деятельности агентов
    ...
    private function getFieldListValue(array $arHlblock, string $fieldName): array
    {
        ...
        if ($fieldID) {
             ...
             */
            $rsFields = CUserFieldEnum::GetList(array(), array(
                "USER_FIELD_ID" => $fieldID,
            ));
            while($arEnum = $rsFields->fetch())
            {
    ...
    private function getAgents(string $entity, array $arTypeAgents): array
    {
        ...
        $nav->allowAllRecords(true)
            ->setPageSize($this->arParams['AGENT_COUNT']) //Нужно передать параметр Количество элементов из мввсива $this->arParams
            ->initFromUri();
        ...
        while ($arAgent = $rsAgents->fetch()) {
             ...
             */
            $arAgent['UF_WORKING'] = $arTypeAgents[$arAgent['UF_WORKING']];
    ...
    public function clickStarAction($agentID)
    {
        ...
        $favorites = CUserOptions::GetOption('mcart_agent', 'options_agents_star');
        if($favorites){
            if(!is_array($favorites)){
                $favorites = [$favorites];
            }
            if (in_array($agentID,$favorites)){
                $key = array_search($agentID, $favorites);
                unset($favorites[$key]);
            }else{
                $favorites[] = $agentID;
            }
        ...
        $check = CUserOptions::SetOption('mcart_agent', 'options_agents_star',$favorites);
        if($check){
